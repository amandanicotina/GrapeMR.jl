function finite_difference_gradient(cost_function, u::Vector{Float64}, ctx::GrapeContext; ε=1e-6)
    grad_fd = zeros(Float64, length(u))
    u_perturbed = copy(u)

    for i in eachindex(u)
        u_perturbed[i] = u[i] + ε
        f_plus = cost_function(u_perturbed, ctx)

        u_perturbed[i] = u[i] - ε
        f_minus = cost_function(u_perturbed, ctx)

        grad_fd[i] = (f_plus - f_minus) / (2ε)

        u_perturbed[i] = u[i]  # Reset
    end
...
Δt = diff(range(0.0, control_field.t_control, length(control_field.B1x) + 1))
cf_manual = deepcopy(control_field)
cf_gd     = deepcopy(control_field)
cf_bfgs   = deepcopy(control_field)

# Build context
ctx = GrapeMR.build_grape_context(control_field, spins, grape_params)

# Initial control vector
u0_parts = []
if ctx.fields_opt["B1x"]
    push!(u0_parts, vec(ctx.control_field.B1x))
end
if ctx.fields_opt["B1y"]
    push!(u0_parts, vec(ctx.control_field.B1y))
end
u = reduce(vcat, u0_parts)

# Allocate analytical gradient buffer
G_analytical = similar(u)

# Compute gradients
GrapeMR.gradient_function_vec!(G_analytical, u, ctx)
G_fd = finite_difference_gradient(GrapeMR.cost_function_vec!, u, ctx)

# Compare
using LinearAlgebra
difference = norm(G_analytical - G_fd) / norm(G_fd)
println("Relative gradient difference: $difference")

using Plots
plot(G_analytical, label="Analytical")
plot!(G_fd, label="Finite Diff", linestyle=:dash)

# plot(G_fd./G_analytical)


# formatter(y) = @sprintf("%.e", y)
# plt = plot(
#     plot(G_analytical*0.0001, lw =2,
#         label = "Analytical",
#         title = "Analytical vs Finite Difference Gradient",
#         ylabel = "Amplitude",
#         legend = :topright),
#     plot(G_fd, lw =2,
...
#         linestyle = :dash,
#         legend = :topright,
#         yformatter = formatter),  # <- Cleaner tick labels
#     layout = (2, 1),
#     xlabel = "Iterations"
# )
# display(plt)
