...
labeloff  = ["T1 = 300ms"]#, "White"];
targetoff = ["min"]
offset = 100           # [Hz]
offset_vals  = collect(-offset:2:offset)
n = 5
B1_vals = collect(range(0.7, stop=1.3, length=n))#length(offset_vals)))
spins_offset = GrapeMR.Spin(M0, [0.3], [0.08], offset_vals, [1.0], targetoff, labeloff)
spins_offset_B1 = GrapeMR.Spin(M0, [0.3], [0.08], offset_vals, B1_vals, targetoff, labeloff)
...
@time for (i, b1) in enumerate(B1_values)
    for (j, b0) in enumerate(B0_values)
        s = GrapeMR.Spin(M0, [0.3], [0.08], b0, b1, targetoff, labeloff)
        mag = forward_propagation(grape_output.control_field, s[1])
        dyn = GrapeMR.Magnetization(mag)
        iso = Isochromat(dyn, s[1])
        cost_matrix[i, j] = cost_function(iso, :target_one_spin)
    end
end

# Plot heatmap
using Plots
B1_plot_vals = collect(range(0.7, stop=1.3, length=length(spins_offset_B1)))
B1_plot_vals = collect(range(-offset, stop=offset, length=length(spins_offset_B1)))
